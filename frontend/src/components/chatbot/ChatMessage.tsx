            ...
            <ReactMarkdown
              ...
              components={{
                ...
                code: ({ node: _node, className, children, ...props }) => (
                  <code
                    {...props}
                    className={`rounded border border-cyan-300/20 bg-slate-900 px-1 py-0.5 text-cyan-200 ${className ?? ""}`}
                  >
                    {children}
                  </code>
                ),
                pre: ({ children }) => (
                  <pre className="overflow-x-auto rounded-xl bg-slate-900 p-3">{children}</pre>
                ),
                li: ({ children, ...props }) => (
