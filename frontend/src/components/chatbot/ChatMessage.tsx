  ...
  return (
    ...
    >
      ...
      <div className={`flex max-w-[85%] flex-col gap-1 ${isAssistant ? "items-start" : "items-end"}`}>
        <div
          className={`relative rounded-2xl px-4 py-2.5 text-sm shadow-lg backdrop-blur-md ${isAssistant
              ...
              components={{
                ...
                code: ({ node, className, children, ...props }) => {
                  ...
                  return (
                    <code
                      {...props}
                      className={`rounded-xl border border-[rgba(102,252,241,0.18)] bg-slate-900 px-1 py-0.5 text-[0.8rem] text-cyan-200 ${!isBlock ? "" : "block overflow-x-auto"
                        } ${className ?? ""}`}
                    >
                      {children}
                    </code>
